 ...
 * 2. Hostname is not a private IP.
  ...
  try {
    ...
    if (isIP(hostname)) {
      ...
      try {
        ...
        // It's safer to allow fetch to handle the connection error if DNS fails,
        // but we can't verify the IP.
        // However, if we can't resolve it, we can't check it.
      }
    }
